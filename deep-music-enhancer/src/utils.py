...
# A gathering of utility functions & classes for use in other parts of the project

def torch2np(*tensors):
    # Converts torch tensors to numpy, multiple inputs allowed
    tensors = list(tensors)
    for i in range(len(tensors)):
        if type(tensors[i]).__module__ == 'torch':
            tensors[i] = tensors[i].detach().cpu().numpy()  # convert
...
def read_audio(path, make_stereo=True):  # default use stereo
    sr, audio = wavfile.read(path)
    audio = audio.T
    if np.issubdtype(audio.dtype, np.int16):  # target datatype
        audio = audio.astype(np.float32) / 32768.0
    if len(audio.shape) == 1:    # if mono, simply copy to make it stereo
...
def snr(x, ref):  # estimated signal, reference pure signal
    # Signal-to-noise ratio for numpy arrays
...
def weights_init_normal(m):
    # Initializes weights randomly using normal distribution
    classname = m.__class__.__name__
    if classname.find("Linear") != -1:
        torch.nn.init.normal_(m.weight.data, 0.0, 0.02)  # weights for Linear Units
    elif classname.find("Conv") != -1:
        torch.nn.init.normal_(m.weight.data, 0.0, 0.02)  # weights for Convolutional layers
    elif classname.find("BatchNorm2d") != -1:
        torch.nn.init.normal_(m.weight.data, 1.0, 0.02)  # weights for Batch Normalization
        torch.nn.init.constant_(m.bias.data, 0.0)  # bias=0



class Flatten(torch.nn.Module):  # this is a model
...
# The 'subpixel' layer
class PixelUpscale(torch.nn.Module):  # this is a model
    """
    1D pixel shuffler. https://arxiv.org/pdf/1609.05158.pdf
    Upscales activations
    Doubles (back) the activation length in upsampling network
    """
...
def makedir(path):  # create folder directory
...
def pad_str_zeros(x, n):
    # Add 0 padding to string to become length=n
    x = str(x)
...
def conv1d_same(in_channels, out_channels, kernel_size, bias=True, dilation=1, pad_type='zero'):
    ...
    if pad_type == 'zero':  # default padding=0
...
def lowpass(sig, cutoff, filter_=('cheby1', 8), sr=44100):  # cutoff should be specified in Hz
    ...
    Keyword Arguments:
        ...
        filter_type {str} -- type of filter to use, only butter and cheby1 are implemented (default: {'butter'})
    
    Returns:
        numpy 1d array -- lowpassed signal
    """
    nyq = sr / 2
    cutoff /= nyq  # use relative frequency (relative to nyquist frequency)
