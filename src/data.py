...
# get lowpass filtered data from raw songs (specify filters)
class DatasetAudio:
    ...
    def __init__(self, file_list, sample_len, cutoff, filters):
        ...
        self.input_len = sample_len  # input length is obviously smaller than full song
        self.cutoff = cutoff  #
        # a list of available filters: (type, order)
        if not isinstance(filters, list):  # typecast filters to a list
            filters = [filters]
    ...
    def __getitem__(self, idx):
        try:
            hq, sr = u.read_audio(self.file_list[idx])  # hq=high-quality audio object

            # take a clip of the full song starting at random location
            x_length = hq.shape[1]
            start_loc = random.randint(0, x_length - self.input_len - 1)
            hq = hq[:, start_loc:start_loc + self.input_len]
            # select a filter randomly from the list
